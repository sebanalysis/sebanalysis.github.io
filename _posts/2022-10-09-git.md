...
- Tree-ish path: means either a branch name, or commit. As opposed to a file-path
...
At any given moment, the entire repository has a __single__ commit checked out. That checked-out commit is known as __'HEAD'__.  The files you are seeing in your repository are checkout-out from the HEAD commit and they are known as the '__working-tree__'. Each branch also has an __index__ which represents the proposed next commit. So you can swap branches without loosing staged changes. But un-staged changes are only present in the current working-tree. So you must stage or commit changes before swapping branch. See a summary of the tracked, staged and un-staged files with `git status`.

The change in 'staged changes' is always with respect to the __HEAD commit__. And the change in un-staged changes is always with respect to the Index . Thus:
- all differences of working tree vs Index can be viewed with `git diff` and;
- all differences of Index vs head can be seen with `git diff --staged`. 
...
- '__Restore-files__'__\*\*__ is when you revert just the __work-tree__ to a previous commit, leaving the index and commit history. 
...
Mode | Commit History | Index | Worktree | Effect
-|-|-|- 
Restore Files| = | = |🔙
Un-Stage Files | = | 🔙| =
Soft-Reset | 🔙| = | = | Moves the HEAD commit pointer backwards, retaining the same staged files. 
Mixed-Reset | 🔙| 🔙|= | Like above, but also unstages files.
Hard-Reset | 🔙|🔙|🔙 | Reset all tracked files to the state at t given commit.
...
This is a typical workflow to hard-reset to commit ###:
...
Command | Meaning
-|-
`git reset --hard` |  Hard reset, whole repo
`git reset -- <path>; git restore <path>` | Hard reset, specific path
`git restore <path>` | Restore files, specific path
`git reset -- <path>` | Un-Stage files, specific path

#### Undo to Any Commit

Here ### refers to the commit hash, which can be located with `git log` or commit ID which can be located with `git reflog`. 

Command | Meaning
-|-
`git reset ### --hard` |  Hard reset, whole repo
`git reset ### -- <path>; git restore <path>` | Hard reset, specific path
`git restore --source=### <path> .` |  Restore-files, specific path
`git reset ### -- <path>` | Mixed-reset, specific path
...
Command | Meaning
-|-
`git reset ### --soft` | Reset commit-history of whole repo back to commit ###
`git reset ### --mixed` | As above but also un-stages files
...
`git rm -rf --cached <path> ; git add <path>` | Un-tracks all files not in `.gitignore`.
...
This is where git restore comes in. It most useful as a way to remove un-staged changes from __specific paths__.  The default flags are `HEAD` and `--worktree`.
...
- Safe: 
    - Switch: `git switch <branch>`  equivalent to `git checkout <branch>`: Switch branch or re-attach head.
...
Typically a workflow merging my branch into main looks like:
