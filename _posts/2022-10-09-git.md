...
`git restore .` | Undo: It undoes un-staged changes in current dir.
...
`git checkout ### -- …` | Browse: Change the worktree to a previous commit only in path `…`. `--` Functions purely as a separator of flags from paths. 
`git restore -source=### -SW  -- ...` | Undo in a folder and record it: like above but also deletes superflous current files at path (-SW means staged, worktree). `--` is just a separator of flags form paths.
...
`git rm -rf --cached .`<br>`git add .` | Un-tracks all files: … (i.e. removes them from git index) then add all the unignored ones.
...
-|-|-|-
`--soft` | 🔙| = | = | Moves the commit pointer backwards, retaining the staging. 
`--mixed` (default) | 🔙| 🔙|= | Like above, but also unstages files.
`--hard` | 🔙|🔙|🔙 | Reset all tracked files to the state at t given commit.
