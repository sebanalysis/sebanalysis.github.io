...
[Regular Expression](https://en.wikipedia.org/wiki/Regular_expression), (a.k.a. RegEx) is a sequence of characters that specifies a search pattern in text. The sequence is called a pattern or expression.
...
Normally, this means opening a file.
...
#### 1. Compile the expression
 ...
 I'll add a group (brackets) to allow us to locate the variants on the matches.

I'll use the expression `r"express(.*?)\b`. All the conventions are explained later.
...
This guide will now talk only about how to make the `r"express(.*?)\b"` part of this ordeal, known as the 'pattern' or 'expression'. I deal with python re functions elsewhere. RegEx is powerful and compelling because it makes otherwise complex find-and-replace, or parsing operations trivial and it is useful in every field of computing.
...
`\N` Is nothing. It is NOT not newline.
`\R` In some flavours of regex (not python re), Matches all [line-break characters](https://sebanalysis.github.io/code/plain-text). Matches a whole `CR LF` as one character but also matches:  `CR`  `LF` and `U+0085` (a.k.a "New Line")
...
In my experience, in multi-line mode, you can only use a single instance of each of these start and end anchors per pattern.
To match line-breaks within an expression you still need to use [\r]?[\n]

In python re but not notepad++, `\A and \Z` refer to start and end of string, even in multiline conditions.
...
[glob](<https://en.wikipedia.org/wiki/Glob_(programming>) is a simpler version of regex with much more limited options, which is used to search for file-names. Globbing is handled by the shell per [POSIX](https://en.wikipedia.org/wiki/POSIX) tradition. Long ago, in UNIX V6, there was a program /etc/glob that would expand wildcard patterns. Soon afterward this became a shell built-in.

The universal syntax is simple:

Wildcard | Meaning and regex
-|-
`*` | Any string but file separator: `[^\]*` in windows or `[^/]` in UNIX-like
