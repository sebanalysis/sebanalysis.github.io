...
Also known as options, these are not placed within the expression string itself. Instead, the flags are entered as arguments to the regex function.

### The most common flags

Flag  | Purpose | Default   | Python Flag `▯` in:<br>`re.compile("pattern", re.▯)`
-|-|-|-
Multi-Line | Make `^` and `$` match the start and end of a line cursor positions | `^` and `$` match only the start and end of string | `re.M` or `re.MULTILINE`
Dot-All | Make `.` match `\n` | `.` doesn't match `\n`| `re.S` or `re.DOTALL`
